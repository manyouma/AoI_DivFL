...
-------------
"""
def create_MNIST_ds_dirichlet(n_clients, samples_train, samples_test, alpha):

    MNIST_train = datasets.MNIST(root="./data", train=True, download=True)
    MNIST_test = datasets.MNIST(root="./data", train=False, download=True)

    dirichlet_trains, dirichlet_tests = [], []
    dirichlet_train_labels = []
    dirichlet_test_labels = []
    
        # Get the total number of 
    mnist_labels = np.zeros((10,))
    for i in np.arange(10):
        mnist_labels[i] = sum(MNIST_train.train_labels == i).numpy()
    #alpha = 0.01
    n_classes = 10
    from numpy.random import dirichlet
    matrix = np.array(dirichlet([alpha] * n_classes, size=n_clients))
    while sum(np.sum(matrix, 0)*samples_train < mnist_labels) != 10:
        matrix = np.array(dirichlet([alpha] * n_classes, size=n_clients))

        
    row_train = np.zeros((n_classes, ), dtype=int)
    row_test = np.zeros((n_classes, ), dtype=int)
    
    for i in range(n_clients):
        #print('client '+str(i))
        # Get the label we want according ot the dirichlet distribution
        train_portion = np.array(matrix[i]*samples_train).astype(int)
        test_portion = np.array(matrix[i]*samples_test).astype(int)
        train_portion[np.argmax(train_portion)]+=samples_train-sum(train_portion)
        test_portion[np.argmax(test_portion)]+=samples_test-sum(test_portion)
        train_labels = []
        test_labels = []
        for i_class in np.arange(n_classes):
            train_labels += [i_class]*train_portion[i_class]
            test_labels += [i_class]*test_portion[i_class]

        row_train, dirichlet_train = get_dirichlet(
            MNIST_train, row_train, i, samples_train, n_classes, train_labels
        )

        row_test, dirichlet_test = get_dirichlet(
            MNIST_test, row_test, i, samples_test, n_classes, test_labels
        )

        dirichlet_trains.append([dirichlet_train])
        dirichlet_tests.append([dirichlet_test])
        dirichlet_train_labels.append(np.array([train_labels]))
        dirichlet_test_labels.append(np.array([test_labels]))

    X_train = np.array(dirichlet_trains)
    X_test = np.array(dirichlet_tests)

    y_train = np.array(dirichlet_train_labels)
    y_test = np.array(dirichlet_test_labels)

    folder = "./data/"
    train_path = f"MNIST_dirichlet{alpha}_train_{n_clients}_{samples_train}.pkl"
    with open(folder + train_path, "wb") as output:
        pickle.dump((X_train, y_train), output)

    test_path = f"MNIST_dirichlet{alpha}_test_{n_clients}_{samples_test}.pkl"
    with open(folder + test_path, "wb") as output:
        pickle.dump((X_test, y_test), output)

class MnistDirichletDataset(Dataset):
    ...
    def __init__(self, file_path, k):
        ...
        with open(file_path, "rb") as pickle_file:
            ...
            self.labels = np.array(dataset[1][k].squeeze())
...
def get_dirichlet(ds, row_0: int, digit: int, samples: int, n_classes: int, vector):
    ...
    for i_sample in np.arange(samples):
       # print('i_sample: ' + str(i_sample) + ' label: ' + str(vector[i_sample]) + ' current_row: ' + str(row))
       # print()
        while ds.train_labels[row[vector[i_sample]]] !=  vector[i_sample]:
            ds.train_labels[row[vector[i_sample]]]
            row[vector[i_sample]] += 1
        dirichlet.append(ds.train_data[row[vector[i_sample]]].numpy())
        row[vector[i_sample]] += 1
            
    return row, dirichlet

def clients_set_MNIST_dirichlet(file_name, n_clients, batch_size=200, shuffle=True):
    """Download for all the clients their respective dataset"""
    print(file_name)

    list_dl = list()
    for k in range(n_clients):
        dataset_object = MnistDirichletDataset(file_name, k)
        #print(f'created_{k}')
...
def partition_CIFAR_dataset(
    ...
    kappa = 0.2
    k_n = np.arange(K)+1
    i_vec = np.power(k_n, -kappa)
    if balanced:
        n_samples = [500] * n_clients
    elif not balanced and train:
        n_samples = n_samples = (i_vec/sum(i_vec)*4900).astype(int)*10
    elif not balanced and not train:
        n_samples = (i_vec/sum(i_vec)*4900).astype(int)*2
...
def get_dataloaders(dataset, batch_size: int, shuffle=True):
    ...
    if dataset == "MNIST_iid":
        ...
        n_clients = 200
        samples_train, samples_test = 250, 40
        ...
        alpha = float(dataset[11:])
        samples_train, samples_test = 250, 40

        file_name_train = f"MNIST_dirichlet{alpha}_train_{n_clients}_{samples_train}.pkl"
        path_train = folder + file_name_train

        file_name_test = f"MNIST_dirichlet{alpha}_test_{n_clients}_{samples_test}.pkl"
        path_test = folder + file_name_test

        if not os.path.isfile(path_train):
            create_MNIST_ds_dirichlet(
    n_clients, samples_train, samples_test, alpha
        )

        list_dls_train = clients_set_MNIST_dirichlet(
            path_train, n_clients, batch_size=batch_size, shuffle=shuffle
        )

        list_dls_test = clients_set_MNIST_dirichlet(
            path_test, n_clients, batch_size=batch_size, shuffle=shuffle
        )
